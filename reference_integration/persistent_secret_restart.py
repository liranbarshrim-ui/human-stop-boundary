...
def start(root,state,deploy,admin,stderr_path):
    ...
    return subprocess.Popen([
        sys.executable,str(root/"staging_server.py"),"--host","127.0.0.1",
        "--port",str(PORT),"--data-dir",str(state),
        "--deploy-secret",deploy,"--admin-secret",admin
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="dar-persistent-secret-") as td:
        root_tmp=Path(td);state=root_tmp/"state";state.mkdir()
        secret_dir=root_tmp/"secrets";secret_dir.mkdir(mode=0o700)
        ...
        stderr_path=root_tmp/"staging.stderr"
        p=start(root,state,deploy,admin,stderr_path)
        try:
