...
def wait_health(url):
    ...
    while time.monotonic() < deadline:
        try:
            ...
            pass
        time.sleep(.1)
    raise RuntimeError("staging service did not start")
...
def main():
    ...
    p = subprocess.Popen(["sudo","-u",staging_user,"--","python3",str(ROOT/"staging_server.py"),"--port",str(PORT),"--data-dir",str(state),"--deploy-secret-file",str(secret),"--admin-secret-file",str(secret)], cwd=ROOT)
    ...
    try:
        ...
        guessed = b"wrong-secret-guess"
        token = forged_token(guessed, forged_id, digest)
