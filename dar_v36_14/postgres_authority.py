...
class PostgresAuthority:
    ...
    def _lock_key(outcome: str) -> int:
        # PostgreSQL advisory locks accept a signed 64-bit integer.
    ...
    def state(self) -> dict[str, Any]:
        with self._connect() as conn:
            ...
            refusals = {
                r["outcome_key"]: [r["epoch"], r["refusal_id"]]
                for r in conn.execute("SELECT * FROM dar_refusals")
            }
            effects = {
                r["idempotency_key"]: {"outcome": r["outcome_key"], "epoch": r["epoch"]}
                for r in conn.execute("SELECT * FROM dar_effects")
            }
            committed = {
                r["outcome_key"]: r["idempotency_key"]
                for r in conn.execute("SELECT outcome_key, idempotency_key FROM dar_effects")
            }
    ...
    def fence(self, outcome: str, epoch: int) -> tuple[int, dict[str, Any]]:
        with self._connect() as conn:
            ...
            conn.execute("""
                INSERT INTO dar_fences(outcome_key, fence) VALUES (%s,%s)
                ON CONFLICT (outcome_key) DO UPDATE SET fence=EXCLUDED.fence
            """, (outcome, epoch))
            return 200, {"ok": True}
    ...
    def refuse(self, outcome: str, epoch: int, refusal_id: str) -> tuple[int, dict[str, Any]]:
        with self._connect() as conn:
            ...
            if epoch < current:
                return 409, {"ok": False, "error": "fence_rollback"}
            # A commit that already won the serialized race is not retroactively erased.
            if conn.execute("SELECT 1 FROM dar_effects WHERE outcome_key=%s", (outcome,)).fetchone():
                conn.execute("""
                    INSERT INTO dar_refusals(outcome_key, epoch, refusal_id) VALUES (%s,%s,%s)
                """, (outcome, epoch, refusal_id))
                return 200, {"ok": True, "idempotent": False, "late": True}
            conn.execute("""
                INSERT INTO dar_fences(outcome_key, fence) VALUES (%s,%s)
                ON CONFLICT (outcome_key) DO UPDATE SET fence=GREATEST(dar_fences.fence, EXCLUDED.fence)
            """, (outcome, epoch))
            conn.execute("""
                INSERT INTO dar_refusals(outcome_key, epoch, refusal_id) VALUES (%s,%s,%s)
            """, (outcome, epoch, refusal_id))
    ...
    def commit(self, outcome: str, epoch: int, idem: str) -> tuple[int, dict[str, Any]]:
        with self._connect() as conn:
            ...
            existing = conn.execute(
                "SELECT outcome_key FROM dar_effects WHERE idempotency_key=%s", (idem,)
            ).fetchone()
            if existing:
            ...
            conn.execute(
                "INSERT INTO dar_effects(idempotency_key, outcome_key, epoch) VALUES (%s,%s,%s)",
                (idem, outcome, epoch),
            )
            return 200, {"ok": True, "idempotent": False}
