...
proposed=SystemState(1,{'alice':{'docs':frozenset({'READ','WRITE'})}},{'docs':GovernanceRule('docs','root')}); params={'path':'via.txt','data':'OK'}; cap=Kernel(Store(str(state),secret),secret,boot_id=boot).issue('alice','docs','WRITE',proposed,'nonce-live',params=params)
