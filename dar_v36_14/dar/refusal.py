...
class RefusalAuthority:
    """Authenticated refusal authority with optional outcome-level fencing.

    Legacy refusals are effect-instance scoped. Strong protected-outcome
    refusals MUST carry an explicit outcome_key supplied by the deployment's
    business protocol; it is never inferred from effect_id or parameters.
    """
    ...
    def issue(self, principal, effect_id, capability_txid, target_epoch=None, refusal_id=None, issued_at=None, outcome_key=None):
        ...
        outcome_key = canonical_outcome_key(outcome_key) if outcome_key is not None else ''
    ...
    def issue_protected(self, principal, effect_id, capability_txid, outcome_key, target_epoch=None, refusal_id=None, issued_at=None):
        if outcome_key is None:
            raise ValueError('protected refusal requires an explicit outcome_key')
    ...
    def verify(self, refusal):
        ...
        try:
            ...
            outcome_key = canonical_outcome_key(refusal.outcome_key) if refusal.outcome_key else ''
