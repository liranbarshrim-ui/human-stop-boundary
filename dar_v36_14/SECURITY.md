...
- **Pending recovery:** `reconcile_pending()` is a bounded recovery mechanism. It requires an external `params_provider` to supply the original parameters, verifies their `params_digest`, consults the adapter's authoritative status, and retries UNKNOWN/PREPARED only through the adapter's idempotent contract. It clears durable pending state only after COMMITTED is established and the journal contains matching validated intent. If the deployment cannot supply the original parameters or the adapter cannot provide authoritative idempotency/status semantics, complete recovery is not established; the intent can remain pending and requires an external operational decision.
