...
class V3613(unittest.TestCase):
    def setUp(self):
        ...
        s=SystemState(0,{'alice':{'docs':frozenset({'READ'})}},{'docs':GovernanceRule('docs','root')}); self.store._write_atomic(Snapshot(0,0,frozenset(),frozenset(),tuple(),s.canonical(),'B'*64))
    def tearDown(self): self.d.cleanup()
    def cap(self):
        p=SystemState(1,{'alice':{'docs':frozenset({'READ'})}},{'docs':GovernanceRule('docs','root')}); return self.k.issue('alice','docs','READ',p,'n1',params={'x':1})
    ...
    def test_journal_prepare_failure_does_not_lose_recoverable_intent(self):
        c=self.cap(); a=Adapter(); j=BrokenJournal('PREPARED')
        with self.assertRaises(OSError): self.g.execute_recoverable(self.req(c),a,{'x':1},j)
        self.assertEqual(a.calls,[]); self.assertEqual(len(self.store._read().pending_effects),1)
    def test_commit_journal_failure_leaves_durable_intent(self):
        c=self.cap(); a=Adapter(); j=BrokenJournal('COMMITTED')
        with self.assertRaises(OSError): self.g.execute_recoverable(self.req(c),a,{'x':1},j)
        s=self.store._read(); self.assertEqual(len(s.pending_effects),1); self.assertEqual(s.pending_effects[0]['params_digest'],__import__('dar.effect_transaction',fromlist=['_params_digest'])._params_digest({'x':1})); self.assertEqual(a.calls,[(f'{c.txid}:e1',{'x':1})])
    def test_reconcile_pending_after_commit_journal_failure(self):
        c=self.cap(); a=Adapter(); j=BrokenJournal('COMMITTED')
        with self.assertRaises(OSError): self.g.execute_recoverable(self.req(c),a,{'x':1},j)
        real=EffectJournal(Path(self.d.name)/'effects.log',b'z'*32); intent=self.store._read().pending_effects[0]; real.append({'status':'PREPARED',**intent}); self.assertEqual(self.g.reconcile_pending(a,real,lambda i:{'x':1}),1); self.assertEqual(self.store._read().pending_effects,()); self.assertEqual(real._validated_state()[intent['key']]['status'],'COMMITTED'); self.assertEqual(a.calls,[(f'{c.txid}:e1',{'x':1})])
