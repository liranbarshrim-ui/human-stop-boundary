...
class LiveCrashBoundary(unittest.TestCase):
    def test_sigkill_rotates_generation_and_rejects_old_capability(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            launcher=Path(__file__).resolve().parents[1]/'run_privileged_server.py'; env=dict(os.environ); env['PYTHONPATH']=str(launcher.parent); cmd=[sys.executable,str(launcher),'--socket',str(ipc/'dar.sock'),'--root',str(root),'--state',str(state),'--secret-file',str(secret_file),'--allowed-uid',str(os.getuid())]
            def start():
                ...
                for _ in range(200):
                    ...
                    if (ipc/'dar.sock').exists():
                        import socket
                        try: q=socket.socket(socket.AF_UNIX,socket.SOCK_STREAM); q.settimeout(.05); q.connect(str(ipc/'dar.sock')); q.close(); return p
                        except OSError: pass
                    time.sleep(.01)
                p.kill(); raise RuntimeError('server did not start')
            p=start()
            try:
                current=Store(state,secret)._read(); params={'path':'missing'}; cap=Kernel(Store(state,secret),secret,boot_id=current.boot_id).issue('alice','docs','READ',SystemState(1,{'alice':{'docs':frozenset({'READ'})}},{'docs':GovernanceRule('docs','root')}),'live-crash-nonce',params=params)
            finally: os.kill(p.pid,signal.SIGKILL); p.wait(timeout=3)
            p2=start()
            try:
                req=EffectRequest(cap,'alice','docs','READ','old-cap','READ')
                with self.assertRaises(BoundaryDenied): UnixDispatcherClient(str(ipc/'dar.sock')).execute(req,params)
            finally: os.kill(p2.pid,signal.SIGTERM); p2.wait(timeout=3)
